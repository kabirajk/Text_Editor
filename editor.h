...
public:
  ...
  void fileEditor() {
    ...
    while (1) {
      ...
      displayText();
      std::cout << std::endl;
      std::cout << "1 => addText, 2 => updateText, 3 => removeText, 4 => "
                   "displayText, 5 => revertVersion 6 => exit "
                << std::endl;
      int option = -1;
      std::cin >> option;
      if (option == 1) {
      ...
      if (option == 5) { //! addText Version count As repeated
        /*RevertVersion*/ revertVersion();
      }
      if (option == 6) { /*break of loop*/
        break;
      }
      ...
      if (count % (5 + 1) == 0) {
        ...
        currentVersion += 1;
        addCommit();
        copystring(currentHead,
                   fileDataList.mutable_filedatabase(currentVersion - 1));
        currentHead->set_commitcount(1);
        setHead();
        count = 1;
      }

      if (count != currentHead->commitcount()) {
        currentHead->set_commitcount(count);
      }
    }
  }

  void printData() {
    std::cout << "[File]: " << currentProject->filename() << " by "
              << currentProject->creater() << std::endl;
  }
  ...
  void updateText() {
    ...
    std::cin >> index;
    if ((index >= 0 && index < currentHead->textline_size()) &&
        currentHead->textline_size() != 0)
  ...
  void removeText() {
    ...
    for (auto it = currentHead->textline().begin();
         it != currentHead->textline().end(); it++) {
      if (std::string(it->data()) == element) {
        currentHead->mutable_textline()->erase(it);
        break;
      };
    }
  }
  void displayText() { // implement empty file
    CLEARSCREEN;
    std::cout << "[" << fileName << "]  [V" << currentVersion << "]"
              << std::endl;
    for (int lineindex = 0; lineindex < currentHead->textline_size();
         lineindex += 1)
      std::cout << lineindex << ". " << currentHead->textline(lineindex)
                << std::endl;
    if (!currentHead->textline_size()) {
  ...
  void revertVersion() {
    ...
    if (fileDataList.filedatabase_size() > 0) {
      ...
      if (versiontohop > 0 &&
          versiontohop <= fileDataList.filedatabase_size()) {
  ...
  void readData(std::string fileversion) {
    std::fstream binObject(fileversion + ".bin",
  ...
  void writeData(std::string fileversion) {
    std::fstream boutObject(fileversion + ".bin",
                            std::ios::out | std::ios::trunc | std::ios::binary);
    if (!boutObject.is_open()) {
      std::cout << "unable to open textfile *.bin file" << std::endl;
    } else if (!fileDataList.SerializePartialToOstream(&boutObject)) {
      std::cout << "unable to write data !data lost" << std::endl;
    }
  }

  ~Editor() {
    writeData(fileName);
    currentProject->set_currentversion(currentVersion);
  }
};
