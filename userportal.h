...
public:
  UserPortal() { // prasing data from bin file using fstream
  ...
  void logIn() {
    ...
    for (userindex; userindex < userListArray.userdatabase_size();
         userindex += 1) {
      const userdata::User &currentuser = userListArray.userdatabase(userindex);
      // std::cout << userListArray.userdatabase_size()
      // <<(username==currentuser.name())<<(
      // password==currentuser.password())<<std::endl;
      if (currentuser.name() == username) { // try to impelent for email
    ...
    if (userListArray.userdatabase_size() == 0)
      signUp();
    else if (notfound == 1) {
  ...
  void signUp() {
    ...
    // std::cout << name << email << password << location << address <<
    // phonenumber << std::endl;
    std::cout << "Unique id is [" << userUniqueCount << "] SignIn Sucessfull"
              << std::endl;
  ...
  ~UserPortal() {
    std::fstream boutObject("UserData.bin",
                            std::ios::out | std::ios::trunc | std::ios::binary);
    if (!boutObject.is_open()) {
      std::cout << "unable to open user *.bin file" << std::endl;
    } else if (!userListArray.SerializePartialToOstream(&boutObject)) {
