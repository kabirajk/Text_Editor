...
public:
  TextEditor(std::string user) {
    ...
    if (!binObject.is_open()) {
      std::cout << "project *.binfile not found creating a new file"
                << std::endl;
  ...
  void createNew() {
    ...
    projectList.set_count(projectList.count() + 1);
    auto edit = Editor(projectList.mutable_projectnamelist(
        projectList.projectnamelist_size() - 1));
    // proced to next()
  }
  void existingFile() {
    display();
    if (projectList.projectnamelist_size() == 0) {
      ...
      if ((fileindex >= 0 && fileindex < projectList.projectnamelist_size()) &&
          projectList.projectnamelist_size() != 0) {
        auto edit = Editor(projectList.mutable_projectnamelist(fileindex));
      } else
        std::cout << "Not a file index: " << fileindex << std::endl;
    }
  }
  void display() {
    CLEARSCREEN;
    ...
    for (int projectindex = 0;
         projectindex < projectList.projectnamelist_size(); projectindex++) {
      std::cout.width(12);
      std::cout << std::left;
      ...
      std::cout.width(30);
      std::cout << projectList.projectnamelist(projectindex).currentversion()
                << std::endl;
    }
    std::cout << std::endl;
  }
  void deleteFile() {
    for (int projectindex = 0;
         projectindex < projectList.projectnamelist_size(); projectindex++) {
      std::cout << projectindex << ". "
                << projectList.projectnamelist(projectindex).filename()
                << std::endl;
    }
    ...
    if (id >= 0 && id < projectList.projectnamelist_size()) {
      ...
      for (auto iter = projectList.mutable_projectnamelist()->begin();
           iter != projectList.mutable_projectnamelist()->end(); iter++) {
      ...
      std::cout << "[File][index] not Matched" << std::endl;
  }

  ~TextEditor() {
    std::fstream boutObject(creater + ".bin",
                            std::ios::out | std::ios::trunc | std::ios::binary);
    if (!boutObject.is_open()) {
      std::cout << "unable to open project *.bin file" << std::endl;
    } else if (!projectList.SerializePartialToOstream(&boutObject)) {
